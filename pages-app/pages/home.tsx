...
export function HomePage() {
  ...
  return <main>
    <section className="relative overflow-hidden border-b border-white/[.055]"><div className="surface-grid pointer-events-none absolute inset-0 opacity-60" /><div className="relative mx-auto max-w-7xl px-5 py-20 text-center sm:py-28 lg:px-8">
      ...
      <h1 className="mx-auto mt-5 max-w-4xl text-[clamp(2rem,9vw,4.5rem)] font-semibold tracking-[-.06em]">Find the perfect<span className="block whitespace-nowrap bg-gradient-to-r from-violet-300 to-cyan-300 bg-clip-text text-transparent">Geometry Dash macro</span></h1>
      ...
      <form onSubmit={search} className="mx-auto mt-9 flex max-w-2xl items-center gap-2 rounded-2xl border border-white/[.09] bg-[#0e1118] p-2 shadow-2xl shadow-violet-950/15"><Search className="ml-3 h-5 w-5 text-zinc-600" /><input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Search macros" placeholder="Macro, level, ID, creator, uploader…" className="h-12 min-w-0 flex-1 bg-transparent px-2 text-sm outline-none placeholder:text-zinc-700" /><button className="h-11 rounded-xl bg-violet-500 px-5 text-xs font-semibold hover:bg-violet-400">Search</button></form>
