...
function Row({ index, href, title, detail, value, profile }: { index: number; href: string; title: string; detail: string; value: string; profile?: ProfileRow }) { return <Link to={href} className="flex items-center gap-3 rounded-xl border border-white/[.045] bg-white/[.018] p-3 hover:bg-white/[.04]"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-white/[.04] text-xs font-semibold text-zinc-500">{index + 1}</span>{profile && <Avatar profile={profile} className="h-8 w-8 rounded-lg text-[10px]" />}<span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold">{title}</span><span className="mt-1 block truncate text-[10px] text-zinc-700">{detail}</span></span><span className="text-[10px] text-zinc-500">{value}</span></Link>; }
