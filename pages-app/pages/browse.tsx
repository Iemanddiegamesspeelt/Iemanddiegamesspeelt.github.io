...
export function BrowsePage() {
  ...
  const filtered = useMemo(() => {
    ...
    const result = (macros ?? []).filter((macro) => {
      ...
      if (format && !macro.available_format_ids.includes(format)) return false;
      if (tool) {
        const compatible = macro.available_format_ids.some((formatId) => formatCompatibilityRegistry.some((entry) =>
          entry.formatId === formatId
          && entry.replayToolId === tool
