...
function ReportMacro({ macro }: { macro: MacroRow }) {
  ...
  const [choice, setChoice] = useState<'working' | 'broken' | 'outdated' | null>(null);
  ...
  useEffect(() => {
    ...
    void supabase().from('macro_reports').select('status').eq('macro_id', macro.id).eq('reporter_id', user.id).maybeSingle().then(({ data }) => {
      if (active) setChoice((data?.status as typeof choice) ?? null);
    });
    return () => { active = false; };
  }, [macro.id, user]);
  ...
  async function report(status: 'working' | 'broken' | 'outdated') {
    ...
    if (error) setMessage(error.message);
      ...
      setCounts((current) => {
        const next = { ...current };
        if (choice) next[choice] = Math.max(0, next[choice] - voteWeight);
        next[status] += voteWeight;
        return next;
      });
      setChoice(status);
      setMessage('Choice saved. You can change it anytime.');
    }
  }
  return <section className={`rounded-[24px] border p-5 ${communityWarning ? 'border-amber-400/20 bg-amber-400/[.055]' : 'border-white/[.075] bg-[#0e1118]'}`}><h2 className="text-sm font-semibold">Does this macro work?</h2>{communityWarning && <p role="alert" className="mt-3 text-xs leading-5 text-amber-100">Community warning: broken and outdated choices outnumber working choices.</p>}<div className="mt-4 grid grid-cols-3 gap-2">{([['working', working], ['broken', broken], ['outdated', outdated]] as const).map(([status, count]) => user ? <button key={status} type="button" disabled={restricted} onClick={() => void report(status)} className={`rounded-lg border px-2 py-2 text-[9px] capitalize transition disabled:cursor-not-allowed disabled:opacity-40 ${choice === status ? 'border-violet-400/35 bg-violet-400/15 text-violet-100' : 'border-white/[.07] bg-white/[.025] text-zinc-400 hover:text-white'}`}>{status}<span className="ml-1 text-zinc-600">{count}</span></button> : <span key={status} className="rounded-lg border border-white/[.06] bg-white/[.02] px-2 py-2 text-center text-[9px] capitalize text-zinc-500">{status}<span className="ml-1 text-zinc-700">{count}</span></span>)}</div>{!user && <Link to={`/login?return_to=${encodeURIComponent(`/macro/${macro.id}`)}`} className="mt-4 inline-flex text-xs font-semibold text-violet-300">Sign in to choose</Link>}{restricted && <p className="mt-3 text-[10px] leading-4 text-amber-200">Voting is unavailable while your account is restricted.</p>}{voteWeight === 10 && <p className="mt-3 text-[10px] leading-4 text-violet-300">Your moderator choice counts as 10.</p>}{message && <p className="mt-3 text-[10px] leading-4 text-zinc-600">{message}</p>}<p className="mt-3 text-[9px] leading-4 text-zinc-700">A community warning needs at least 10 choices.</p></section>;
}
