...
function ReportMacro({ macro }: { macro: MacroRow }) {
  ...
  async function report(status: 'working' | 'broken' | 'outdated') {
    ...
    if (error) setMessage(error.message);
      ...
      setCounts((current) => {
        const next = { ...current };
        if (choice) next[choice] = Math.max(0, next[choice] - 1);
        next[status] += 1;
        return next;
      });
      setChoice(status);
