...
export function MacroCard({ macro, showLevel = true }: { macro: MacroRow; showLevel?: boolean }) {
  return <article className="card-hover rounded-[22px] border border-white/[.075] bg-[#0e1118] p-5">
    ...
    <Link to={`/profile/${macro.uploader?.username ?? macro.uploader_id}`} className="mt-3 flex items-center gap-2 truncate text-[11px] text-zinc-600 hover:text-violet-200"><Avatar profile={macro.uploader} className="h-5 w-5 rounded-md text-[8px]" />@{macro.uploader?.username ?? 'player'}</Link>
