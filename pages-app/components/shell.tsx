...
import { Compass, Crown, FolderHeart, LogIn, Menu, Repeat2, Search, ShieldCheck, Upload, UserRound, X } from 'lucide-react';
    ...
    <div className="min-h-screen">
      <header className="sticky top-0 z-50 border-b border-white/[.065] bg-[#080a0f]/88 backdrop-blur-xl">
        ...
        {open && <div className="border-t border-white/[.06] bg-[#0a0d13] p-3 lg:hidden"><div className="mx-auto grid max-w-7xl gap-1">
          ...
          {!user && <Link to="/login" onClick={() => setOpen(false)} className="mt-2 flex items-center justify-center gap-2 rounded-xl bg-violet-500 px-4 py-3 text-sm font-semibold"><LogIn className="h-4 w-4" />Sign in</Link>}
        </div></div>}
      </header>
      <Outlet />
